...
export default function EmployeeDashboardPage() {
  ...
  const handleSidebarNavigate = (id) => {
    if (id === 'employee-schedule' || id === 'my-schedule') {
      navigate('/employee/schedule');
    } else if (id === 'weekly-schedules' || id === 'schedules') {
      navigate('/store-manager/schedules');
    } else if (id === 'kiosk-codes') {
    ...
    <DashboardShell
      sidebar={
        <DashboardSidebar
          ...
          activePath="/employee/schedule"
          onNavigate={handleSidebarNavigate}
          defaultDisplayName={storedUser?.fullName || 'Nhân Viên'}
          roleLabel={storedUser?.roleName || storedUser?.role || 'Nhân Sự Cửa Hàng'}
          avatarLetter={storedUser?.fullName?.charAt(0) || 'N'}
          brandName="RWFM PORTAL"
      ...
      topbar={
        <DashboardTopbar
          page="employee-schedule"
          pageTitles={{ 'employee-schedule': 'Lịch Làm Việc Cá Nhân' }}
          consoleLabel="EMPLOYEE PORTAL"
          defaultDisplayName={storedUser?.fullName || 'Nhân Viên'}
          roleLabel={storedUser?.roleName || storedUser?.role || 'Nhân Sự'}
          avatarLetter={storedUser?.fullName?.charAt(0) || 'N'}
          fallbackTitle="Lịch Làm Việc Cá Nhân"
          homePath="/employee/schedule"
        />
      }
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        {/* Page Header */}
        <PageHeader
          index="Cổng nhân sự · Lịch phân công"
          title="LỊCH LÀM VIỆC CÁ NHÂN"
          desc={`Theo dõi ca trực tuần được công bố tại ${storedUser?.storeName || 'Cửa hàng Tiện lợi Chi nhánh Cầu Giấy'}.`}
          actions={
            <Button kind="primary" variant="primary" icon="refresh" onClick={fetchMyShifts} loading={loading}>
              Làm mới lịch
            </Button>
          }
        />
                    ...
                    {hasShift ? (
                      day.shifts.map((s, idx) => {
                        ...
                        return (
                          ...
                          >
                            <div style={{ fontWeight: 700, fontSize: 14, color: badgeColor, marginBottom: 5 }}>
                              {s.shiftName}
                            </div>
                            <div style={{ fontSize: 12.5, color: c.fg, display: 'flex', alignItems: 'center', gap: 6 }}>
                            ...
                            <div style={{ fontSize: 12, color: c.fgSubtle, marginTop: 4 }}>
                      ...
                      <div style={{ padding: '24px 0', textAlign: 'center', color: c.fgFaint, fontSize: 12.5, fontStyle: 'italic' }}>
